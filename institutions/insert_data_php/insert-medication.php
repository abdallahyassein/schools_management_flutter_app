...
$student_id = 0;
$name = $explanation =$time_of_medication="";
$name_err = $explanation_err = $student_id_err =$time_of_medication_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["name"]))) {
    $name_err = "Please insert illness name.";
  } else {
  ...
  if (empty(trim($_POST["explanation"]))) {
   ...
   // Validate Time of medication
  if (empty(trim($_POST["timeofmedication"]))) {
    $time_of_medication_err = "Please insert the time of medication.";
  } else {
    $time_of_medication = trim($_POST["timeofmedication"]);
  }
  ...
  if (
    ...
    $sql = "INSERT INTO medication (name,description,time_of_treatment,school_id,student_id) VALUES (?, ?, ?, ?, ?)";


    if ($stmt = mysqli_prepare($link, $sql)) {
      // Bind variables to the prepared statement as parameters
      mysqli_stmt_bind_param($stmt, "ssssi", $param_name, $param_explanation,$param_time_of_medication,$param_school_id, $param_student_id);

      // Set parameters
      $param_name = $name;
      $param_explanation = $explanation;
      $param_time_of_medication = $time_of_medication;
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        header("location: insert-medication.php");
      } else {
...
<head>
  <meta charset="utf-8">
  <title>Insert Medication Case</title> <!-- title for the page -->
          ...
          <div class="card bg-light mb-3">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="form-group <?php echo (!empty($name_err)) ? 'has-error' : ''; ?>">
                  <label>Illness</label>
                  <input type="text" name="name" class="form-control" value="<?php echo $name; ?>" placeholder="Enter Illness Name">
                ...
                <div class="form-group <?php echo (!empty($explanation_err)) ? 'has-error' : ''; ?>">
                  ...
                  <input type="text" name="explanation" class="form-control" value="<?php echo $explanation; ?>" placeholder="Enter Explanation for the illness">
                  <span class="help-block" style="color:red"><?php echo $explanation_err; ?></span>
                </div>

                <div class="form-group <?php echo (!empty($time_of_medication_err)) ? 'has-error' : ''; ?>">
                  <label>Time Of Medication </label>
                  <input type="text" name="timeofmedication" class="form-control" value="<?php echo $time_of_medication; ?>" placeholder="Enter a time for the medication">
                  <span class="help-block" style="color:red"><?php echo $time_of_medication_err; ?></span>
                </div>
